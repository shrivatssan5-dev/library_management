...
export async function migrate() {
  ...
  try {
    ...
    const { rows } = await client.query(
      'SELECT id FROM librarians WHERE email = $1',
      ['librarian@library.com']
    );

    if (rows.length === 0) {
      const passwordHash = await bcrypt.hash('librarian123', 10);
      await client.query(
        'INSERT INTO librarians (email, password_hash, name) VALUES ($1, $2, $3)',
        ['librarian@library.com', passwordHash, 'Default Librarian']
      );
