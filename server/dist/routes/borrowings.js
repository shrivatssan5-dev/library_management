...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const bookResult = await client.query('SELECT copies_available FROM books WHERE id = $1 FOR UPDATE', [book_id]);
        ...
        const { rows } = await client.query(`INSERT INTO borrowings (book_id, member_id, librarian_id, due_date)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [book_id, member_id, librarianId, due_date]);
        await client.query('UPDATE books SET copies_available = copies_available - 1 WHERE id = $1', [book_id]);
...
router.post('/:id/return', async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const borrowing = await client.query(`SELECT * FROM borrowings WHERE id = $1 FOR UPDATE`, [req.params.id]);
        ...
        const { rows } = await client.query(`UPDATE borrowings
       SET status = 'returned', returned_at = NOW()
       WHERE id = $1
       RETURNING *`, [req.params.id]);
        await client.query('UPDATE books SET copies_available = copies_available + 1 WHERE id = $1', [record.book_id]);
        await client.query('COMMIT');
        res.json(rows[0]);
    }
    catch (error) {
