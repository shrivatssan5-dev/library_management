...
router.get('/', async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query(`SELECT * FROM members
...
router.post('/', async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query(`INSERT INTO members (name, email, phone, membership_number)
       VALUES ($1, $2, $3, $4)
       RETURNING *`, [
            name.trim(),
...
router.put('/:id', async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query(`UPDATE members
...
router.delete('/:id', async (req, res) => {
    try {
       ...
       WHERE member_id = $1 AND status IN ('active', 'overdue')`, [req.params.id]);
        if (active.rows[0].count > 0) {
            return res.status(400).json({ error: 'Cannot delete a member with active borrowings' });
        }
        const { rowCount } = await db_1.pool.query('DELETE FROM members WHERE id = $1', [req.params.id]);
        if (rowCount === 0) {
            return res.status(404).json({ error: 'Member not found' });
        }
        res.status(204).send();
